...
data_list = readExcelData.getExceldata(config.api_data_path,config.sh8)
logger = logging.getLogger("mainlog.unfinishorder")
...
class TestunfinishOrder(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        logger.info("{}接口用例页开始".format(config.sh8))
    ...
    def tearDownClass(cls):
        ...
        logger.info("{}接口用例页结束".format(config.sh8))
    ...
    def testselectOrder(self,testdata):
        ...
        try:
            ...
            if expect_result == "ok":
                self.assertEqual(exist[0][0],"已发货",msg="数据库中订单状态错误")
                logger.info("测试结果为Pass")
                write_exceldata.writeExcelData(datafile=config.api_data_path,sheetname=config.sh8,row=int(testdata["Id"]),column=len(testdata)-1,value="Pass")
                self.DataBase.updata_db(sql="update indent set o_state='已付款' where account='fu'") #数据库环境复原
            else:
                self.assertNotEqual(exist[0][0],"已发货",msg="数据库中订单状态遭改变")
                logger.info("测试结果为Pass")
                write_exceldata.writeExcelData(datafile=config.api_data_path,sheetname=config.sh8,row=int(testdata["Id"]),column=len(testdata)-1,value="Pass")
        except Exception as e:
            logger.info("断言异常，测试结果为Fail")
            write_exceldata.writeExcelData(datafile=config.api_data_path,sheetname=config.sh8,row=int(testdata["Id"]),column=len(testdata)-1,value="Fail")
            if exist[0][0] == "已发货":
                 self.DataBase.updata_db(sql="update indent set o_state='已付款' where account='fu'") #数据库环境复原
            raise e
        finally:
            write_exceldata.writeExcelData(datafile=config.api_data_path,sheetname=config.sh8,row=int(testdata["Id"]),column=len(testdata)-2,value=res.text)
